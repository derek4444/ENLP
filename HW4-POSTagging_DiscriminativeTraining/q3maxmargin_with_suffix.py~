...
N = 0
testlines = [[] for index in range(40)]
sufflen = 2
...
def updateFeatureValues(lineno,worsttagsequence) :
	...
	global tagtagcount
	global wordtagcount
	global wordsuffixtagcount	
	global suffixprobmap	
	worstwordtagcount = dict()
	worsttagtagcount = dict()
	worstwordsuffixcount = dict()
	worstwordsuffixcount.clear()
	...
	for i in range(0,len(inputlines[lineno])) : 
		key = inputlines[lineno][i][-sufflen:] + '_' + worsttagsequence[i]
		if key not in worstwordsuffixcount :
			worstwordsuffixcount[key] = 1
		else :
			worstwordsuffixcount[key] += 1
	...
	for element1 in taglist :
		for element2 in wordlist :
			...
			key1 = element2[-sufflen:] + '_' + element1
			...
			if key1 in worstwordsuffixcount :
				term3 = worstwordsuffixcount[key1]
			else :
				term3 = 0
...
#############################################################
			if key1 in wordsuffixtagcount:
				regterm = ((eeta * regular_constant) * suffixprobmap[key1]/N)
				suffixprobmap[key1] = suffixprobmap[key1] + (eeta * (wordsuffixtagcount[key1] - term3)) - regterm
			else :
				regterm = ((eeta * regular_constant) * suffixprobmap[key1]/N)
				suffixprobmap[key1] = suffixprobmap[key1] + (eeta * (0 - term3)) - regterm
################################################################
...
def clearWordTagCounts(lineno) :
	...
	for i in range(0,len(inputlines[lineno])) :  
		...
		key1 = inputlines[lineno][i][-sufflen:] + '_' + taglines[lineno][i]
		wordtagcount[key] = 0
		wordsuffixtagcount[key1] = 0
...
#########################################################################################
	for i in range(0,len(inputlines[lineno])) :  
		key = inputlines[lineno][i][-sufflen:] + '_' + taglines[lineno][i]
		if key not in wordsuffixtagcount :
			wordsuffixtagcount[key] = 1
		else :
			wordsuffixtagcount[key] += 1
#########################################################################################
...
def applyRevisedValues(lineno) :
	...
	for i in range(2,stateid[lineno]-taglength+1) :
		...
		if characterindex < taglength :
			...
			key5 = inputline[(i - 2)/(2*taglength)][-sufflen:] + '_' + key2
			if key4 == key2 :			
				lossvalue = 0.0
				fvalue = 1
			else :
			...
			#########################################################################			
			if key5 in wordsuffixtagcount :
				edgecost[i][0] = edgecost[i][0] + fvalue * suffixprobmap[key5]	
			########################################################################		
		else :
...
def createArrayGraph(inputline,lineno) :
	...
	for i in range(2,stateid[lineno]-taglength+1) :
		...
		if stateindex < taglength :
			...
			key5 = inputline[tagindex1][-sufflen:] + '_' + key2
			if key4 == key2 :
				lossvalue = 0.0
				fvalue =  1
				term1 = fvalue * wordprobmap[key3] + lossvalue
			else :
				lossvalue = 1.0
				fvalue = 0
				term1 = lossvalue
			#########################################################################
			if key5 in wordsuffixtagcount :
				term2 = term1 + fvalue * suffixprobmap[key5]
				secondpart.append(term2)
			else :
				term2 = term1
				secondpart.append(term2)		
			#####################################################################3
...
################################	
	for element1 in wordlist :
		for element2 in taglist :
			...
			key2 = element1[-sufflen:] + '_' + element2
			wordprobmap[key1] = 0
			suffixprobmap[key2] = 0
##############################
	tag1 = 'S'	
	for tag2 in taglist :
		key1 = tag1 + '_' + tag2
...
def createDecoderLattice(inputline) :
	...
	for i in range(2,totalnodes-taglength) :
		...
		if stateindex < taglength :
			...
			key2 = taglist[tagindex]
			key3 = inputline[tagindex1] + '_' + key2
			key4 = inputline[tagindex1][-sufflen:] + '_' + key2

			if flag == 1 :
				if key3 in wordtagcount :
					fvalue =  1
					term1 = fvalue * wordprobmap[key3]
				else :
					term1 = float('-inf')
			#######################################################
				if key4 in wordsuffixtagcount :
					fvalue = 1
					term2 = fvalue * suffixprobmap[key4]
				else :
					term2 = float('-inf')

				secondpart.append(term1 + term2)
			######################################################
